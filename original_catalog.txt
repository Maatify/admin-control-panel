...
### 48.2 دلالات المخزون للحجز (Inventory Semantics for Reservations)
**ملاحظة توضيحية هامة:** داخل Catalog V1، يمثل الحقل `quantity_on_hand` **الرصيد الحالي المتاح للبيع للعملاء (Current Sellable Inventory Balance)**، والذي يستخدم لحساب إتاحة المنتج.
ومع ذلك، الكتالوج **ليس** موديولاً مخصصاً للحجز:
- لا يحتوي الكتالوج على سجلات حجوزات (Reservation records).
- لا يحتوي الكتالوج على مستودعات (Warehouses) أو دفتر حركة مخزون (Movement ledger).
- دورة حياة الحجز وهوية الحجز تُدار بالكامل خارج الكتالوج (مثلاً في موديول Orders). يقتصر دور الكتالوج على كشف آلية إنقاص وتعديل رصيد المخزون للبيع.
