/**
 * 🌍 City Translations V2 (Bridge-first)
 */
...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    if (!window.AdminPageBridge) {
      console.error('❌ AdminPageBridge missing for city_translations-v2');
      return;
    }
    ...
    if (!window.cityTranslationsContext) {
      console.error('❌ Missing window.cityTranslationsContext');
      return;
    }

    const apiEndpoints = window.cityTranslationsApi || {};
    const context = window.cityTranslationsContext;
    const capabilities = window.cityTranslationsCapabilities || {};
    const cityId = context.city_id;
    ...
    const containerId = window.cityTranslationsTableContainerId || 'translations-table-container';
    let currentPage = 1;
    let currentPerPage = 20;
    const reloadCityTranslationsTableV2 = function() {
      return loadTable();
    };
    ...
    function initLanguageDropdown() {
      const languages = (window.cityTranslationsContext || {}).languages || [];
      if (!languages.length) {
    ...
    const modalCityDisplay = document.getElementById('edit-city-display');
    ...
    function openEditModal(languageId, languageName, languageCode, translatedName) {
      ...
      modalCityDisplay.textContent = context.city_code;
    ...
    Bridge.Events.onClick('.btn-delete-translation', function(event, deleteBtn) {
      ...
      Bridge.API.runMutation({
        ...
        endpoint: 'geo/cities/' + cityId + '/translations/delete',
        method: 'POST',
        ...
        reloadHandler: reloadCityTranslationsTableV2
      });
    });
    ...
    if (btnSaveTranslation) {
      btnSaveTranslation.addEventListener('click', function() {
        ...
        Bridge.API.runMutation({
          ...
          endpoint: 'geo/cities/' + cityId + '/translations/upsert',
          method: 'POST',
          payload,
    ...
    window.reloadCityTranslationsTableV2 = reloadCityTranslationsTableV2;
    window.CityTranslationsV2 = {
      reload: reloadCityTranslationsTableV2
    };
  });
})();
