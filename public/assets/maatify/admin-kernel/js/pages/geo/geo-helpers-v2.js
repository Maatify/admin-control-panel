...
(function() {
    ...
    console.log('🛠️ Geo Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-geo-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const geoId = btn.getAttribute(dataAttribute);
            if (requireData && !geoId) {
                console.warn('[GeoHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(geoId, btn, event);
        });
    }
    ...
    console.log('✅ GeoHelpersV2 loaded');
})();
