/**
 * 🛠️ Countries Helpers V2
 * Bridge-first helper utilities for countries pages.
 */

(function() {
    'use strict';

    console.log('🛠️ Countries Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-country-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const countryId = btn.getAttribute(dataAttribute);
            if (requireData && !countryId) {
                console.warn('[CountriesHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(countryId, btn, event);
        });
    }
    ...
    console.log('✅ CountriesHelpersV2 loaded');
})();
