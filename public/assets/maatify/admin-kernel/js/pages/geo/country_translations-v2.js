/**
 * 🌍 Country Translations V2 (Bridge-first)
 */
...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    if (!window.AdminPageBridge) {
      console.error('❌ AdminPageBridge missing for country_translations-v2');
      return;
    }
    ...
    if (!window.countryTranslationsContext) {
      console.error('❌ Missing window.countryTranslationsContext');
      return;
    }

    const apiEndpoints = window.countryTranslationsApi || {};
    const context = window.countryTranslationsContext;
    const capabilities = window.countryTranslationsCapabilities || {};
    const countryId = context.country_id;
    ...
    const containerId = window.countryTranslationsTableContainerId || 'translations-table-container';
    let currentPage = 1;
    let currentPerPage = 20;
    const reloadCountryTranslationsTableV2 = function() {
      return loadTable();
    };
    ...
    function initLanguageDropdown() {
      const languages = (window.countryTranslationsContext || {}).languages || [];
      if (!languages.length) {
    ...
    const modalCountryDisplay = document.getElementById('edit-country-display');
    ...
    function openEditModal(languageId, languageName, languageCode, translatedName,cauntryNamw) {
      ...
      modalCountryDisplay.textContent = cauntryNamw;
    ...
    Bridge.Events.onClick('.btn-delete-translation', function(event, deleteBtn) {
      ...
      Bridge.API.runMutation({
        ...
        endpoint: 'geo/countries/' + countryId + '/translations/delete',
        method: 'POST',
        ...
        reloadHandler: reloadCountryTranslationsTableV2
      });
    });
    ...
    if (btnSaveTranslation) {
      btnSaveTranslation.addEventListener('click', function() {
        ...
        Bridge.API.runMutation({
          ...
          endpoint: 'geo/countries/' + countryId + '/translations/upsert',
          method: 'POST',
          payload,
    ...
    window.reloadCountryTranslationsTableV2 = reloadCountryTranslationsTableV2;
    window.CountryTranslationsV2 = {
      reload: reloadCountryTranslationsTableV2
    };
  });
})();
