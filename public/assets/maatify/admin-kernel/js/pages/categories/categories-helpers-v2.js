/**
 * 🛠️ Categories Helpers V2
 * Bridge-first helper utilities for categories pages.
 */

(function() {
    'use strict';

    console.log('🛠️ Categories Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-category-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const categoryId = btn.getAttribute(dataAttribute);
            if (requireData && !categoryId) {
                console.warn('[CategoriesHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(categoryId, btn, event);
        });
    }
    ...
    console.log('✅ CategoriesHelpersV2 loaded');
})();
