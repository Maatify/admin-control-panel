...
(function() {
    ...
    if (!window.AdminPageBridge || typeof LanguagesHelpersV2 === 'undefined') {
        console.error('❌ LanguagesHelpersV2 not found! Make sure languages-helpers.js is loaded first.');
        return;
    }
    ...
    const ApiHandler = {
        ...
        showAlert: function(type, message) {
            if (type === 'danger') return Bridge.UI.error(message);
            if (type === 'warning') return Bridge.UI.warning(message);
            if (type === 'success') return Bridge.UI.success(message);
            return Bridge.UI.info(message);
        },
        showFieldErrors: function(errors, formId) {
            if (window.ApiHandler && typeof window.ApiHandler.showFieldErrors === 'function') {
                window.ApiHandler.showFieldErrors(errors, formId);
            }
        }
    };
    ...
    function initModals() {
        ...
        // Setup close handlers - use LanguagesHelpersV2
        LanguagesHelpersV2.setupModalCloseHandlers();
    ...
    // ========================================================================
    // Modal Controls (deprecated - use LanguagesHelpersV2 instead)
    // ========================================================================

    /**
     * @deprecated Use LanguagesHelpersV2.closeAllModals() instead
     */
    ...
    /**
     * @deprecated Use LanguagesHelpersV2.openModal() instead
     */
    function openModal(modalId) {
        LanguagesHelpersV2.openModal(modalId);
    }
    ...
    function setupCreateLanguageForm() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            if (typeof loadLanguages === 'function') {
                loadLanguages();
            } else if (window.languagesDebugV2 && window.languagesDebugV2.loadLanguages) {
    ...
    async function openEditSettingsModal(languageId) {
        ...
        LanguagesHelpersV2.openModal('edit-settings-modal');
    }
    ...
    function setupEditSettingsForm() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            // Reload languages table
            if (window.languagesDebugV2 && window.languagesDebugV2.loadLanguages) {
                window.languagesDebugV2.loadLanguages();
            }
        });
    }
    ...
    window.openCreateLanguageModalV2 = openCreateLanguageModal;
    window.openEditSettingsModalV2 = openEditSettingsModal;
