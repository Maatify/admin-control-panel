/**
 * 🛠️ Website UI Themes Helpers V2
 */
...
(function() {
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-theme-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const id = btn.getAttribute(dataAttribute);
            if (cfg.requireData !== false && !id) return;

            await callback(id, btn, event);
        });
    }
    ...
    window.WebsiteUiThemesHelpersV2 = {
        setupButtonHandler,
