/**
 * 🛠️ Exchange Rates Helpers V2
 * Bridge-first helper utilities for rates pages.
 */

(function() {
    'use strict';

    console.log('🛠️ Rates Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-rate-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const rateId = btn.getAttribute(dataAttribute);
            if (requireData && !rateId) {
                console.warn('[RatesHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(rateId, btn, event);
        });
    }
    ...
    console.log('✅ RatesHelpersV2 loaded');
})();
