/**
 * 🛠️ Exchange Rate Providers Helpers V2
 * Bridge-first helper utilities for providers pages.
 */

(function() {
    'use strict';

    console.log('🛠️ Providers Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-provider-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const providerId = btn.getAttribute(dataAttribute);
            if (requireData && !providerId) {
                console.warn('[ProvidersHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(providerId, btn, event);
        });
    }
    ...
    window.ProvidersHelpersV2 = {
        ...
        clearFormInputs,
        setFormDisabled,
        createResetPageReload: bindResetPageReload,
        bindResetPageReload,
        bindTableActionState
    };

    console.log('✅ ProvidersHelpersV2 loaded');
})();
