/**
 * 🛠️ Cities Helpers V2
 * Bridge-first helper utilities for cities pages.
 */

(function() {
    'use strict';

    console.log('🛠️ Cities Helpers V2 Loading...');
    ...
    function setupButtonHandler(selector, callback, options) {
        const cfg = options || {};
        const dataAttribute = cfg.dataAttribute || 'data-city-id';
        ...
        return Bridge.Events.onClick(selector, async function(event, btn) {
            ...
            const cityId = btn.getAttribute(dataAttribute);
            if (requireData && !cityId) {
                console.warn('[CitiesHelpersV2] Missing data attribute', { selector, dataAttribute });
                return;
            }

            await callback(cityId, btn, event);
        });
    }
    ...
    console.log('✅ CitiesHelpersV2 loaded');
})();
