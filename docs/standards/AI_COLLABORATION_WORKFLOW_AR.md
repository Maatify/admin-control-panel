...
هذا الملف هو **المصدر الوحيد للحقيقة للقواعد العامة** الخاصة بتقسيم الأدوار وإدارة التنفيذ والمراجعة بين مالك المشروع، والمساعد القائد، والمنفذ المحلي (Local Executor)، وJules.
...
- Technical Lead.
- Architect ضمن القرارات المعتمدة.
- Work Coordinator.
- Prompt Author.
...
- إعادة بناء الحالة الحالية من المستودع.
...
- كتابة توجيه تنفيذي مغلق الحدود وبالحد الأدنى الكافي من المعلومات.
- مراجعة التقرير والـ patch والـ PR.
- تصحيح عنوان أو وصف الـ PR عند توفر الصلاحية.
- تصنيف النتيجة: جاهزة، تحتاج تعديلًا، تحتاج follow-up، أو يجب إيقافها.

### 3.2.1 حدود تنفيذ المساعد القائد

الوضع الافتراضي أن المساعد القائد **لا ينفذ تغييرات المستودع بنفسه** طالما يوجد منفذ مخصص للمهمة.

- Runtime implementation يذهب إلى المنفذ المحلي افتراضيًا.
- Documentation/audit/roadmap work يذهب إلى Jules افتراضيًا.
- تعديل PR metadata ومراجعة GitHub جزء من دور المساعد القائد عند توفر الصلاحية.
- لا ينفذ المساعد القائد كودًا أو توثيقًا داخل المستودع إلا بتكليف صريح من مالك المشروع.
...
المنفذ المحلي (مثل Codex أو أي منفذ آخر يصرح به مالك المشروع) هو **منفذ الكود المحلي الأساسي**. بيئة المنفذ المحلي المعتمدة **توفر** `git`، و `gh`، و `Docker` / `Docker Compose`، وأدوات لغة المشروع والاختبارات والتحليل. يستخدم المنفذ الأدوات المتاحة وفق حاجة المهمة ولا يُطلب منه تثبيتها كافتراض ابتدائي، وإذا كانت أداة مطلوبة غير متاحة يتوقف ويعرض الدليل.
...
Jules هو **منفذ التوثيق والتدقيق والـ roadmap والمهام الصغيرة المعزولة**.
...
Jules لا يقرر سياسة أو معمارية من نفسه، ولا يحول مهمة توثيقية إلى Runtime change دون تصريح.

---

# 4. مصفوفة اختيار المنفذ

| نوع المهمة | المنفذ الافتراضي | ملاحظات |
|---|---|---|
| Runtime implementation | المنفذ المحلي | مع اختبارات وتحليل ساكن |
| Service/Repository/Controller | المنفذ المحلي | يحتاج checkout محليًا ومراجعة patch |
| Contracts/DTOs/Enums | المنفذ المحلي | مع حماية public API |
| Unit/Integration tests | المنفذ المحلي | بنتائج فعلية |
| Lifecycle documentation review | Jules | يجب مقارنته بالكود الحالي |
| Audit جديد أو تحديث audit | Jules | evidence-based فقط |
| Roadmap update | Jules | لا يغير الحالة دون دليل |
| Docs-only correction صغير | Jules | branch وPR مستقلان خاصان بالمهمة |
| PR title/body correction | المساعد القائد | بعد التحقق من remote HEAD |
| Final PR review | المساعد القائد | diff وscope وchecks وbase freshness |
| Merge decision | مالك المشروع | لا ينتقل لأي وكيل |

تُخالف هذه الاختيارات الافتراضية فقط عندما يذكر التوجيه سببًا واضحًا أو يطلب مالك المشروع ذلك صراحة.
...
إذا كان Runtime والاختبارات سليمين ولم يتبق إلا تصحيح توثيقي صغير غير مؤثر:

1. يمكن اعتماد Runtime ودمجه بقرار مالك المشروع.
2. يُعزل التصحيح في Documentation-only follow-up.
3. يُغلق الـ follow-up قبل مرحلة لاحقة ستعدل الملف نفسه.

لا تُستخدم follow-up لإخفاء bug حقيقي أو test failure أو contract مكسور.
...
- Merge يظل قرارًا مستقلًا لمالك المشروع ولا يُنفذ دون تصريح مباشر خاص بالدمج.
...
إذا فقدت session الاستيعاب أو تحولت جولات التصحيح إلى إصلاح آثار الجولات السابقة:

1. تُفصل النتيجة السليمة عن الجزء الفاشل أو غير المكتمل.
2. يجوز دمج الجزء السليم فقط إذا كان مستقلًا، متماسكًا، ويجتاز بوابات القبول، وبقرار مالك المشروع.
3. تبدأ session جديدة من الـ baseline المعتمدة وتستلم المتبقي فقط.
4. إذا لم توجد حالة سليمة مستقلة، لا يُدمج الفشل لتسهيل البدء؛ تُستخدم آخر baseline authoritative سابقة.
...
- المنفذ المحلي للكود المحلي والاختبارات.
- Jules للتوثيق والتدقيق والمهام الصغيرة المعزولة.
- المساعد القائد لا ينفذ داخل المستودع إلا بطلب صريح.

## المرحلة 4 — بناء التوجيه وإعداد Jules Session
...
لا يُعتمد تقرير المنفذ دون مراجعة الـ staged patch أو الـ PR عندما تكون متاحة.

## المرحلة 8 — Commit وPush وPR

تحدث فقط وفق تصريح مكتوب من مالك المشروع أو تصريح واضح داخل المهمة، وكل تصحيح بعد Commit يُضاف في Commit جديد. نشر Jules واختيار Publish Branch أو Publish PR يخضعان للقسم `6.7`.
...
تُعزل الملاحظات الصغيرة غير المانعة، وتُغلق قبل أي مهمة لاحقة تتداخل معها. لا يعني ذلك فتح session أو PR جديدة تلقائيًا؛ يُستخدم المسار المتتابع فقط عند تحقق سيناريو التكدس الوارد في القسم `6.7`.
...
تُعزل في مهمة مستقلة عندما تكون:

- صغيرة.
- غير مانعة لصحة Runtime.
- خارج acceptance criteria الأساسية.
- لا تسبب تداخلًا مع مهمة لاحقة قبل إغلاقها.

لا تستخدم لإخفاء test failure أو Runtime bug أو contract مكسور.
...
يعتمد المعيار نموذج **Central Canonical + Pinned Local Copy**:

1. يُحفظ هذا المعيار كنسخة محلية مثبتة في المشروع داخل:

```text
docs/standards/AI_COLLABORATION_WORKFLOW_AR.md
```

2. يجب أن تبدأ النسخة المحلية بترويسة (metadata) تسجل:
   - Upstream Repository: Maatify/php-engineering-standards
   - Upstream Path: standards/ai/AI_COLLABORATION_WORKFLOW_AR.md
   - Upstream Version: الإصدار المثبت
3. يمنع الاعتماد على floating `main` من المستودع المركزي.
4. تتم أي ترقية للإصدار عبر PR مستقلة تراجع أثر التغيير على المشروع.
5. أضف في `AGENTS.md` الجذري رابطًا إلزاميًا للقراءة.
6. ضع القواعد الخاصة بالمشروع فقط داخل `AGENTS.md`.
7. اعتمد Review Staging أو سجل استثناء المشروع بوضوح.
8. لا تنسخ القواعد العامة في ملفات متعددة.
9. اجعل الـ Prompts تشير إلى المسارات المرجعية بدل نسخ محتواها.
10. تأكد أن أي Prompt يستخدم النسخة المدموجة والمثبتة من المعيار.
...
- **تعثر session:** عند فقدان الاستيعاب يُعتمد آخر جزء سليم فقط إن كان مستقلًا ومقبولًا، ثم تبدأ session جديدة لإكمال المتبقي.
...
- **الأدلة:** مطلوبة قبل قبول أي ادعاء.
- **التبعيات وعدم التداخل:** يسبقان الترتيب الرقمي.
- **هذا الملف:** المصدر الواحد للحقيقة للقواعد العامة للعملية.
