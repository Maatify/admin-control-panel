...
## 1.5 Code Classification Definitions
To accurately apply the "Reality Overrides Rules" protocol, the agent MUST differentiate between:
- **Canonical Code**: Code that perfectly adheres to the current execution system and should be used as a template for new features.
- **Legacy Tech Debt**: Code that works but violates current execution rules. It MUST NOT be used as a template for new features, but constitutes "Reality" for its own execution boundary during migrations or minor updates.
