-- =============================================================================
-- Schema: maa_image_profiles
-- Package: maatify/image-profile
-- =============================================================================
--
-- Design principles:
...
CREATE TABLE `maa_image_profiles`
(
    `id`                    INT UNSIGNED     NOT NULL AUTO_INCREMENT,
    `code`                  VARCHAR(64)      NOT NULL                    COMMENT 'Stable business identifier used in all API calls',
    `display_name`          VARCHAR(128)     DEFAULT NULL                COMMENT 'Human-readable label for administrative UIs',
    ...
    `notes`                 TEXT             DEFAULT NULL                COMMENT 'Internal notes for administrative use; not exposed to end users',
    ...
    `requires_transparency` TINYINT(1)       NOT NULL DEFAULT 0          COMMENT '1 = only PNG/WebP accepted (alpha-capable formats); 0 = no restriction',
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uq_maa_image_profiles_code` (`code`),
    KEY `idx_maa_image_profiles_is_active` (`is_active`)

) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci
    COMMENT = 'Reusable image validation profiles — maatify/image-profile';

-- =============================================================================
-- Migration: add extension columns to an existing base schema
-- =============================================================================
-- Run this block if upgrading from an earlier base schema:
--
-- ALTER TABLE `maa_image_profiles`
