...
class BehaviorTraceRecorder
{
    ...
    public function record(
            ...
            try {
                ...
                if (!$this->policy->validateMetadataSize($json)) {
                    if ($this->fallbackLogger) {
                        $this->fallbackLogger->warning('BehaviorTrace metadata exceeded limit. Dropping metadata.', [
                            'action' => $action,
                            'size' => strlen($json)
                        ]);
                    }
                 ...
                 if ($this->fallbackLogger) {
                        $this->fallbackLogger->warning('BehaviorTrace metadata JSON encoding failed.', [
                            'action' => $action,
                            'error' => $e->getMessage()
                        ]);
                    }
        ...
        try {
            ...
            if ($this->fallbackLogger) {
                $this->fallbackLogger->error('BehaviorTrace logging failed', [
