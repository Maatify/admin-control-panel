...
final readonly class PdoI18nDomainUpdater implements I18nDomainUpdaterInterface
{
    ...
    public function existsById(int $id): bool
    {
        $stmt = $this->pdo->prepare(
            'SELECT 1 FROM i18n_domains WHERE id = :id LIMIT 1'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE i18n_domains SET is_active = :is_active WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT code FROM i18n_domains WHERE id = :id'
        );
        $stmt->execute(['id' => $id]);
    ...
    {
        $stmt = $this->pdo->prepare(
            'UPDATE i18n_domains SET code = :code WHERE id = :id'
        );
    ...
    public function repositionSortOrder(
        ...
        try {
            ...
            $stmt = $this->pdo->prepare(
                'SELECT sort_order FROM i18n_domains WHERE id = :id'
            );
            $stmt->execute(['id' => $id]);
    ...
    public function updateMetadata(
        ...
        $sql = sprintf(
            'UPDATE i18n_domains SET %s WHERE id = :id',
            implode(', ', $fields)
        );
